...
def plot_map(my_row):
    ...
    color_list = ['blue','red','orange','purple', 'green']
    for i, val in enumerate(my_row['country']):
        ...
        if i < len(my_row['country'])-1:
            location_next = (my_row['latitude'][i+1]+((i+1)%3)/1000, my_row['longitude'][i+1]+((i+1)%4)/1000)
...
def advanced_find(name,title,auths_db, titles_db, auth_train,title_train,indexed_journeys_df):
    ...
    for i, row in select_list.iterrows():
        if len(row.author_x) == 0:
            select_list.loc[i,'author_x'] = str(row.author_x) + '/n ' + str(row.title)
        elif isinstance(row.author_x,str):
            select_list.loc[i,'author_x'] = str(row.author_x) + '/n ' + str(row.title)
        else: 
            select_list.loc[i,'author_x'] = str(row.author_x[0]) + '/n ' + str(row.title[0])
